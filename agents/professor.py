    ...
    def _prompt(
        ...
        *,
        ...
        return (
            ...
            "For record_grounded mode, write retrieval queries (up to the "
            "allowed maximum); for other modes, queries must be empty. Build "
            "each query from concrete clinical terms that appear verbatim in "
            "records — anatomy, imaging modality, test/panel/analyte names, "
            "medication names, diagnosis terms — never generic meta-phrases "
            "like 'health records', 'medical history', or 'lab results'. Match "
            "a query's language to the records, but you cannot know that "
            "language before searching and it often differs from the question — "
            "medical and lab/imaging records frequently use English or Latin "
            "terminology. So for the most important concepts, search in BOTH the "
            "question's language and English: multilingual embeddings retrieve "
            "across languages, while matching the records' language sharpens "
            "lexical (BM25) ranking. Use terse keyword phrases (3-7 terms), not "
            "sentences, and keep one clinical concept per query. Decompose a "
            "broad or whole-health question into several single-concept facet "
            "queries, one per body system, test type, or study. Expand "
            "abbreviations and add common synonyms and variant spellings (full "
            "term plus acronym)."
